...
public class ComingSoonFragment : Fragment() {
    lateinit var mcontext: Context
    lateinit var tvNomovie: TextView

    companion object {
        lateinit var comingSoonItem: List<ComingSoon>
        fun newInstance(resp: List<ComingSoon>) = ComingSoonFragment().apply {
            comingSoonItem = resp
        }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        try {
            if (comingSoonItem.size > 0) {
                ...
                val mAdapter = MoviesAdapter(comingSoonItem, mcontext)
